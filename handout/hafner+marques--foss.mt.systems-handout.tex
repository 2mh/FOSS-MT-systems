...
seien sie webbasiert, wie Google Translate\footnote{\urld[2011-11-24]{http://translate.google.com/}}, oder für den stationären persönlichen oder unternehmsweiten Einsatz gedacht, 
...
welchen Korpus sich hinter dem System verbirgt. Sind bei einem CSS-System wenigstens die Ressourcen im Sinne von
Open Source (frei) nutzbar, so ist der Spielraum bereits grösser (vgl. \cite{forcada} ab Kapitel 2.1).
...
\begin{itemize}
	\item[Shallow-Transfer] Im einfachsten Fall sucht Apertium im Quellsprachentext nach Sequenzmustern von
	...
	\item[Advanced Transfer] Seit das Sprachpaar Englisch-Katalanisch unterstützt wird, erfolgt die
	...
	\end{enumerate}
	\item[XML] Die linguisischen Daten werden im XML-Format gehalten (vgl. ebd. S.114ff.), aus Gründen
...
Da es unter der LGPL\footnote{\urld[2011-11-24]{http://www.gnu.org/licenses/lgpl.html}} lizenziert ist, kann es als (kosenfreie) Basis für
...
\subsubsection{Technologien}
\begin{itemize}
  \item[Beam search] Moses verwendet den ``Beam search''-Algorithmus, um im Rahmen
  seiner SBMT-Ansätzen effizient zu einem Ergebnis zu kommen (vgl. \cite{norvig}).
...
Machine Translation).
  \item[Factored] Moses verwendet zudem Regeln, um flektierende
    und agglutinierende Sprachen, wie Klingonisch, so zu verarbeiten, dass
    die einzelnen Teile übersetzt werden können; dafür muss die Morphologie der
    Sprache bekannt sein. Dazu können Regeln definiert werden, was aufzeigt, dass
    Moses ebenso einen RBMT-Ansatz unterstützt.
  \item[Confusion networks] Als Input können auch ``Confusion networks'' bzw. WFST (weighted
    finite state transducer) verwendet werden, aus z. B. einem
    Speech2Text-System (vgl. den Moses-Wikiartikel\footnote{\urld[2011-11-24]{http://www.statmt.org/moses/?n=Moses.ConfusionNetworks}} zu dieser Thematik).
\end{itemize}
